...
namespace ALWTTT.Music.Voice
{
    ...
    {
        [Header("Harness")]
        [SerializeField] private bool showTuningPanel = false;
        ...
        public struct ExprContextInputs
        {
            public Tonality tonality; public NoteName rootNote;
            public TimeSignature timeSignature; public int bpm;
        }

        private ExprContext BuildExprContext(ExprContextInputs pc)
        {
            if (pc.bpm <= 0) return default;
        ...
        public void ApplyProfile(VoiceProfileSO p)
        {
            if (p == null) return;
            characterEnabled = true;
            looseness = p.looseness; vibratoDepth = p.vibratoDepth;
            vibratoSpeedHz = p.vibratoSpeedHz; diction = p.diction;
            mouth = (int)p.mouth; brightness = p.brightness;
            transposeSemitones = p.transposeSemitones;
            tensenessAtVel0 = p.tensenessAtVel0; tensenessAtVel127 = p.tensenessAtVel127;
            vibratoDelaySeconds = p.vibratoDelaySeconds; vibratoRampSeconds = p.vibratoRampSeconds;
            pitchLeadSeconds = p.pitchLeadSeconds; leadFullInterval = p.leadFullInterval;
            minLoudness = p.minLoudness; gain = p.gain;
            syncTrimMs = p.startTrimMs;      // D1=A residual constant lives on the profile
            loopPlayback = false;            // integration invariant: the Director re-arms per loop
        }

        public bool ArmFromStemBytes(byte[] stemBytes, ExprContextInputs inputs, VoiceProfileSO profile)
        {
            Stop(); EnsureVoiceMatchesRateMode(); ApplyProfile(profile);
            MidiFile file;
            try { using var ms = new System.IO.MemoryStream(stemBytes); file = MidiFile.Read(ms); }
            catch (Exception ex) { Fail("Stem parse failed: " + ex.Message); return false; }
            ...
            if (list.Count == 0) { Fail("Stem has zero notes."); return false; }
            Arm(list, "stem", BuildExprContext(inputs), startImmediately: false);
            return true;
        }
        ...
        private volatile bool _armedWaiting;
        public bool IsArmedWaiting => _armedWaiting;
        public bool IsSinging => _playing;

        private double _dspStartTime;
        private bool _anchored;

        private void Arm(List<NoteEvt> list, string label, 
            ExprContext ctx, bool startImmediately = true)
        {
            ...
            _sustainTest = false;
            if (startImmediately) { _playing = true; }
            else { _armedWaiting = true; _dspStartTime = double.MaxValue; }
        }

        public void StartAtDspTime(double dspStart)
        {
            if (!_armedWaiting) return;
            _dspStartTime = dspStart;   // write anchor before the flag flip
            _playing = true;            // audio thread reads _playing first
        }
        ...
        private void OnAudioFilterRead(float[] data, int channels)
        {
            ...
            if (_armedWaiting)
            {
                ...
                if (now + (double)(data.Length / channels) / _sampleRate <= _dspStartTime)
                { for (int i = 0; i < data.Length; i++) data[i] = 0f; return; }
                if (!_anchored)
                {
                    _clock = (now - _dspStartTime) - syncTrimMs / 1000.0;
                    _noteOnClock = _clock; _anchored = true; _armedWaiting = false;
                }
            }
