...
{
    /// <summary>
    /// Deterministic <see cref="ILLMClient"/> test double for the card LLM
    /// pipeline (CE-L1 B2). Consumer-side copy of MidiGenPlay's
    /// <c>FakeLLMClient</c> (copy-then-unify, same doctrine as D-L4.3): the
    /// package's test assembly is not referenceable from a consumer, so the
    /// double is duplicated rather than shared. Keep behavior aligned with the
    /// MidiGenPlay original if either changes.
    /// </summary>
    /// <remarks>
    ...
    /// this double sits on the real call path, not a bypass.</para>
