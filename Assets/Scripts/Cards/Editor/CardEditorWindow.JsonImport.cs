...
namespace ALWTTT.Cards.Editor
{
    public sealed partial class CardEditorWindow : EditorWindow
    {
        ...
        // CE-L1 B3: PartEffect assets resolved from composition.modifierEffectNames
        ...
        // path and this JSON box parse and stage the exact same shape.
        // See CardImportDtos.cs / CardImportDtoParser.cs.
        ...
        {
            // Moved verbatim to the shared parser (CE-L1 B1) so Generate and
            // Import converge on one parse surface.
            return CardImportDtoParser.TryParse(json, out dtos, out error);
        }
        ...
        private bool TryStageCardFromDto(CardJsonImport dto, out string error)
        {
            ...
            // ---------------------------------------------------------------------
            // CE-L1 B3: resolve modifierEffectNames -> PartEffect assets BEFORE apply.
            // Exact case-insensitive name match; missing or ambiguous names FAIL the
            // staging (never skipped) — this is the LLM route's only modifier channel.
            _stagedResolvedModifierEffects = null;
            if (_stagedJsonPayload is CompositionCardPayload &&
                dto.composition != null &&
                dto.composition.modifierEffectNames != null &&
                dto.composition.modifierEffectNames.Length > 0)
            {
                if (!TryResolveModifierEffectNames(
                        dto.composition.modifierEffectNames,
                        out _stagedResolvedModifierEffects,
                        out var modifierError))
                {
                    error = modifierError;
                    DiscardStagedJson();
                    return false;
    ...
    out string err)
        ...
        private void ApplyCompositionJson(SerializedObject pso, CompositionJson cj)
        {
            ...
            if (effectsProp != null)
            {
                ...
                // CE-L1 B3: name-resolved modifier effects (resolved + hard-validated
                // in TryStageCardFromDto; by this point every entry exists and is unique).
                if (_stagedResolvedModifierEffects != null)
                {
                    for (int i = 0; i < _stagedResolvedModifierEffects.Length; i++)
                    {
                        if (_stagedResolvedModifierEffects[i] == null) continue;

                        int idx = effectsProp.arraySize;
                        effectsProp.InsertArrayElementAtIndex(idx);
                        effectsProp.GetArrayElementAtIndex(idx).objectReferenceValue =
                            _stagedResolvedModifierEffects[i];
                    }
                }
            }
        }
        ...
        /// <summary>
        /// Resolve PartEffect asset NAMES to assets by exact case-insensitive
        /// match over every PartEffect in the project (CE-L1 B3). All-or-nothing:
        /// a missing name fails with the available list, a name matching two or
        /// more assets fails as ambiguous. Never skips an entry silently.
        /// </summary>
        private static bool TryResolveModifierEffectNames(
            string[] names, out PartEffect[] resolved, out string error)
        {
            resolved = null;
            error = null;

            // Scan once: name -> assets (a name may collide across folders).
            var byName = new System.Collections.Generic.Dictionary<
                string, System.Collections.Generic.List<PartEffect>>(
            ...
            foreach (var guid in AssetDatabase.FindAssets("t:PartEffect"))
            {
                ...
                var asset = AssetDatabase.LoadAssetAtPath<PartEffect>(path);
                if (asset == null) continue;

                if (!byName.TryGetValue(asset.name, out var list))
                {
                    list = new System.Collections.Generic.List<PartEffect>();
                    byName.Add(asset.name, list);
                    allNames.Add(asset.name);
                }
                list.Add(asset);
            }

            var output = new System.Collections.Generic.List<PartEffect>();
            foreach (var raw in names)
            {
                ...
                {
                    allNames.Sort(System.StringComparer.Ordinal);
                    error = $"modifierEffectNames: no PartEffect asset named '{name}'. " +
                            (allNames.Count == 0
                                ? "No PartEffect assets exist in the project."
                ...
                if (matches.Count > 1)
                {
                    ...
                    foreach (var m in matches) paths.Add(AssetDatabase.GetAssetPath(m));
                    error = $"modifierEffectNames: name '{name}' is ambiguous — " +
                            $"{matches.Count} PartEffect assets share it: " +
                            $"{string.Join(", ", paths)}. Rename one or reference by hand.";
                    return false;
                }
        ...
        private void SaveStagedJsonToAssetsAndAddToCatalog()
        {
            ...
            try
            {
                ...
                // CE-L1 B3 (D-CE-L1.6): consume any pending LLM field plan now that
                // the assets exist on disk — creates the role bundle and assigns the
