...
namespace ALWTTT.Cards.Editor
{
    public sealed partial class CardEditorWindow : EditorWindow
    {
        ...
        // CE-E1: role preset for the create wizard. Holds a TrackRole *name* set
        // by the role-specific "New Rhythm/Backing/Melody/Harmony" buttons
        // (null/empty = no role preset). Resolved by name (like
        // ResolveBundleTypeForRole) so it never compile-couples to specific
        // TrackRole members. Consumed by CreateCardFromWizard to set the
        // composition role and auto-mint a typed style bundle.
        [SerializeField] private string _presetRoleName;
            ...
            using (new EditorGUILayout.HorizontalScope())
            {
                if (GUILayout.Button(_createWizardOpen ? "Hide Create Card" : "Create Card...", GUILayout.Height(22)))
                {
                    ...
                    if (_createWizardOpen)
                    {
                        ...
                        _presetRoleName = null; // plain wizard: no role preset
                    }
                }
    ...
    _catalogSource == CatalogSource.Generic))
        ...
        private void CreateCardFromWizard()
        {
            ...
            // CE-E1 role preset: set the composition role + auto-mint a typed
            // style bundle (mirrors the inspector's "Create" style-bundle button).
            if (!string.IsNullOrEmpty(_presetRoleName) && created.cardDefinition.CompositionPayload != null)
                ApplyPresetRoleAndBundle(created.cardDefinition.CompositionPayload, _presetRoleName);
        ...
        // -----------------------------------------------------------------
        // CE-E1: Clone Card + New-Card presets
        // -----------------------------------------------------------------

        /// <summary>
        /// Clone the selected card into a fully independent card: a new
        /// CardDefinition asset, a deep-copied payload asset, and — for
        /// Composition cards — a CLONED style-bundle asset. (Manual Ctrl+D
        /// leaves the duplicate pointing at the SOURCE bundle, so editing the
        /// copy's palette/override mutates the original; this is the bug we fix.)
        /// New id/displayName are derived from the source.
        /// </summary>
        private void CloneSelectedCard()
        {
            if (_loadedCatalog == null)
                return;
            ...
            var sourceCard = entry.card;
            var sourcePayload = sourceCard.Payload;

            string sourceCardPath = AssetDatabase.GetAssetPath(sourceCard);
            if (string.IsNullOrEmpty(sourceCardPath))
            {
                EditorUtility.DisplayDialog("Clone Card",
                    "The selected card is not a saved asset, so it cannot be cloned.", "OK");
                return;
            }

            string cardFolder = Path.GetDirectoryName(sourceCardPath)?.Replace("\\", "/");
            if (string.IsNullOrEmpty(cardFolder))
            {
                EditorUtility.DisplayDialog("Clone Card",
                    "Could not resolve the source card's folder.", "OK");
                return;
            }

            // 1) Clone identity (D2): "<name> (Clone)" + a catalog-unique id.
            string sourceDisplay = new SerializedObject(sourceCard)
                .FindProperty("displayName")?.stringValue;
            if (string.IsNullOrWhiteSpace(sourceDisplay))
                sourceDisplay = sourceCard.Id;

            string cloneDisplay = $"{sourceDisplay} (Clone)";
            string cloneId = MakeUniqueCloneId(sourceCard.Id);
            string safeBase = ToSafeToken(cloneId, "Card");

            // 2) Deep-copy the payload asset (Instantiate => own serialized copy;
            //    SerializeReference effect data is copied, shared asset references
            //    such as palettes stay shared by design). Lands beside the source.
            CardPayload clonedPayload = null;
            if (sourcePayload != null)
            {
                string sourcePayloadFolder = Path.GetDirectoryName(
                    AssetDatabase.GetAssetPath(sourcePayload))?.Replace("\\", "/");
                if (string.IsNullOrEmpty(sourcePayloadFolder))
                    sourcePayloadFolder = $"{cardFolder}/Payloads";
                EnsureFolderExists(sourcePayloadFolder);

                clonedPayload = UnityEngine.Object.Instantiate(sourcePayload);
                string payloadPath = AssetDatabase.GenerateUniqueAssetPath(
                    $"{sourcePayloadFolder}/{safeBase}_Payload.asset");
                AssetDatabase.CreateAsset(clonedPayload, payloadPath);
            }

            // 3) Clone the card asset, repoint at the cloned payload, set id/name.
            //    Filenames match the post-wizard convention ({id}, {id}_Payload).
            var clonedCard = UnityEngine.Object.Instantiate(sourceCard);
            string clonedCardPath = AssetDatabase.GenerateUniqueAssetPath(
                $"{cardFolder}/{safeBase}.asset");
            AssetDatabase.CreateAsset(clonedCard, clonedCardPath);

            var clonedCardSo = new SerializedObject(clonedCard);
            SetString(clonedCardSo, "id", cloneId);
            SetString(clonedCardSo, "displayName", cloneDisplay);
            SetObject(clonedCardSo, "payload", clonedPayload);
            clonedCardSo.ApplyModifiedPropertiesWithoutUndo();

            EditorUtility.SetDirty(clonedCard);
            if (clonedPayload != null)
                EditorUtility.SetDirty(clonedPayload);
            AssetDatabase.SaveAssets();
            AssetDatabase.Refresh();

            // 4) Clone the referenced style bundle (the Ctrl+D bug fix).
            TrackStyleBundleSO clonedBundle = null;
            if (clonedPayload is CompositionCardPayload clonedComposition)
                clonedBundle = CloneAndRepointStyleBundle(clonedComposition);

            // 5) Register in the catalog, mirroring the source entry's acquisition
            //    settings. Clean up the new assets if the add is rejected.
            if (!MusicianCatalogService.TryAddEntry(
                    _loadedCatalog,
                    clonedCard,
                    entry.flags,
                    ...
                    out var addErr))
            {
                string cleanupCardPath = AssetDatabase.GetAssetPath(clonedCard);
                string cleanupPayloadPath = clonedPayload != null
                    ? AssetDatabase.GetAssetPath(clonedPayload) : null;
                string cleanupBundlePath = clonedBundle != null
                    ? AssetDatabase.GetAssetPath(clonedBundle) : null;

                if (!string.IsNullOrEmpty(cleanupBundlePath))
                    AssetDatabase.DeleteAsset(cleanupBundlePath);
                if (!string.IsNullOrEmpty(cleanupPayloadPath))
                    AssetDatabase.DeleteAsset(cleanupPayloadPath);
                if (!string.IsNullOrEmpty(cleanupCardPath))
                    AssetDatabase.DeleteAsset(cleanupCardPath);
                AssetDatabase.SaveAssets();
                AssetDatabase.Refresh();

                EditorUtility.DisplayDialog("Clone Card",
                    "Could not add the clone to the catalog (new assets were removed):\n\n" + addErr,
                    "OK");
                return;
            }

            _selectedEntryIndex = newIndex;
            Selection.activeObject = clonedCard;
            EditorGUIUtility.PingObject(clonedCard);
            Repaint();
        }

        /// <summary>
        /// Clone the style bundle referenced by a freshly cloned composition
        /// payload into its own asset and repoint the payload at the copy, so the
        /// clone never shares a bundle with its source. Returns the new bundle (or
        /// null if the source had none). Mirrors CreateAndAssignStyleBundle's
        /// folder/naming so cloned bundles look like freshly-created ones.
        /// </summary>
        private TrackStyleBundleSO CloneAndRepointStyleBundle(CompositionCardPayload clonedPayload)
        {
            if (clonedPayload == null)
                return null;

            var so = new SerializedObject(clonedPayload);
            so.Update();

            var trackActionProp = so.FindProperty("trackAction");
            var roleProp = trackActionProp?.FindPropertyRelative("role");
            var styleBundleProp = trackActionProp?.FindPropertyRelative("styleBundle");
            if (styleBundleProp == null)
                return null;

            // After Instantiate this still points at the SOURCE bundle.
            var sourceBundle = styleBundleProp.objectReferenceValue as TrackStyleBundleSO;
            if (sourceBundle == null)
                return null; // nothing assigned — leave the clone's bundle null too

            string roleName = GetEnumNameSafe(roleProp);
            string folder = GetDefaultStyleBundleFolder(clonedPayload, roleName);
            EnsureFolderExists(folder);

            string fileName = $"{clonedPayload.name}_{roleName}_StyleBundle.asset";
            string assetPath = AssetDatabase.GenerateUniqueAssetPath($"{folder}/{fileName}");

            var clonedBundle = UnityEngine.Object.Instantiate(sourceBundle);
            AssetDatabase.CreateAsset(clonedBundle, assetPath);
            AssetDatabase.SaveAssets();
            AssetDatabase.Refresh();

            styleBundleProp.objectReferenceValue = clonedBundle;
            so.ApplyModifiedProperties();
            EditorUtility.SetDirty(clonedPayload);
        ...
        /// <summary>
        /// Derive a catalog-unique id for a clone: "<sourceId>_Clone", then
        /// "_Clone_02", "_Clone_03" … if that is already taken in the loaded catalog.
        /// (Asset filenames are separately de-duplicated via GenerateUniqueAssetPath.)
        /// </summary>
        private string MakeUniqueCloneId(string sourceId)
        {
            string baseId = string.IsNullOrWhiteSpace(sourceId) ? "Card" : sourceId.Trim();

            string candidate = $"{baseId}_Clone";
            if (!CatalogHasCardId(candidate))
                return candidate;

            for (int n = 2; n < 1000; n++)
            {
                string c = $"{baseId}_Clone_{n:D2}";
                if (!CatalogHasCardId(c))
                    return c;
            }

            // Extremely unlikely fallback.
            return $"{baseId}_Clone_{Guid.NewGuid():N}";
        }

        private bool CatalogHasCardId(string id)
        {
            var entries = _loadedCatalog?.Entries;
        ...
        /// <summary>
        /// Row of one-click presets over the existing create wizard. Action /
        /// Composition just set the kind; the role presets also stash a role so
        /// CreateCardFromWizard auto-mints a correctly-typed style bundle.
        /// </summary>
        private void DrawNewCardPresetButtons()
        {
            bool canCreate = _catalogSource == CatalogSource.Musician
                             && _loadedCatalog != null
                             && _loadedMusicianData != null;
            ...
            using (new EditorGUILayout.VerticalScope(EditorStyles.helpBox))
            {
                GUILayout.Label("New Card (presets)", EditorStyles.miniBoldLabel);

                using (new EditorGUILayout.HorizontalScope())
                {
                    if (GUILayout.Button("Action"))
                        BeginCreatePreset(CardAssetFactory.CreateCardKind.Action, null, "Action");

                    if (GUILayout.Button("Composition"))
                        BeginCreatePreset(CardAssetFactory.CreateCardKind.Composition, null, "Composition");
                }

                using (new EditorGUILayout.HorizontalScope())
                {
                    if (GUILayout.Button("Rhythm"))
                        BeginCreatePreset(CardAssetFactory.CreateCardKind.Composition, "Rhythm", "Rhythm");

                    if (GUILayout.Button("Backing"))
                        BeginCreatePreset(CardAssetFactory.CreateCardKind.Composition, "Backing", "Backing");

                    if (GUILayout.Button("Melody"))
                        BeginCreatePreset(CardAssetFactory.CreateCardKind.Composition, "Melody", "Melody");
        ...
        /// <summary>
        /// Open the create wizard pre-filled for a preset. Role presets stash the
        /// role (consumed on Create) and seed the name tag so the suggested id
        /// regenerates for the chosen role.
        /// </summary>
        private void BeginCreatePreset(
            CardAssetFactory.CreateCardKind kind, string roleName, string tag)
        {
            _createWizardOpen = true;
            _newKind = kind;

            _presetRoleName = string.IsNullOrEmpty(roleName) ? null : roleName;
        ...
        /// <summary>
        /// Apply a preset role to a freshly created composition payload and mint a
        /// correctly-typed style bundle for it (reuses CreateAndAssignStyleBundle).
        /// </summary>
        private void ApplyPresetRoleAndBundle(CompositionCardPayload payload, string roleName)
        {
            if (payload == null || string.IsNullOrEmpty(roleName))
                return;
            ...
            {
                Debug.LogWarning(
                    "[CardEditorWindow] Could not apply preset role: " +
                    "CompositionCardPayload.trackAction is missing 'role'/'styleBundle'.");
                return;
            }

            // Resolve by name (like ResolveBundleTypeForRole) so we never depend on a
            // specific TrackRole member existing. If this enum has no such member the
            // card is still created — just without a role/bundle preset.
            if (!SetEnumPropByName(roleProp, roleName))
            {
                Debug.LogWarning(
                    $"[CardEditorWindow] TrackRole has no '{roleName}' member; " +
                    "created the card without a role/bundle preset.");
                return;
            }

            so.ApplyModifiedProperties();

            // Fresh cards have no bundle yet; mint one for the chosen role.
            if (styleBundleProp.objectReferenceValue == null)
                CreateAndAssignStyleBundle(roleProp, styleBundleProp, payload);
        }

        private static bool SetEnumPropByName(SerializedProperty enumProp, string name)
        {
            if (enumProp == null || enumProp.propertyType != SerializedPropertyType.Enum)
                return false;

            int idx = Array.IndexOf(enumProp.enumNames, name);
            if (idx < 0)
                return false;
