...
namespace ALWTTT.Status
{
    ...
    public sealed class StatusEffectSO : ScriptableObject
    {
        ...
        /// <summary>
        /// [R5 / F-R4-3] Public seam for editor tooling (StatusEffectWizardWindow).
        /// Exposes the SAME suggestion + normalization that OnValidate applies, so a
        /// key written by a tool and a key generated by the inspector can never
        /// diverge in format. Duplicating the normalization in the wizard would
        /// create a second source of truth for the shape of a serialization key.
        /// Editor-only by containment: this whole region is #if UNITY_EDITOR.
        /// </summary>
        public static string SuggestKey(string input) => GenerateSuggestedKey(input);
