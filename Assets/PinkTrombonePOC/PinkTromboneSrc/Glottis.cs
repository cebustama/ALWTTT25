namespace Vocal
{
    ...
    internal class Glottis
    {
        ...
        // POC-FORK(6): upstream added two simplex jitter terms to F0
        // (0.02·S(4.07t) + 0.04·S(2.15t)) outside BOTH the AutoWobble gate and
        // VibratoAmount — an always-on ±~90-cent wander at 2–4 Hz that no public
        // property could reach. This gain scales them; 1 = upstream parity.
        float pitchJitterGain = 1;
        public float PitchJitterGain
        {
            get => this.pitchJitterGain;
            set => this.pitchJitterGain = Check01(value);
        }

        // POC-FORK(7): same pattern on tenseness — upstream drifts it by
        // 0.1·S(0.46t) + 0.05·S(0.36t) (±~0.11), wandering Rd, spectral tilt and
        // waveformLoudness with no gate. This gain scales it; 1 = upstream parity.
        float tensenessJitterGain = 1;
        public float TensenessJitterGain
        {
            get => this.tensenessJitterGain;
            set => this.tensenessJitterGain = Check01(value);
        }
        ...
        // POC-FORK(1): upstream had a single `loudness` field, overwritten by
        // SetupWaveform every glottal cycle (tenseness^0.25), making the public
        // Loudness property write-only in practice. Split into:
        //   waveformLoudness — internal, tenseness-derived, keeps stock timbre;
        //   userLoudness     — caller-owned amplitude (MIDI velocity), default 1.
        float waveformLoudness = 1;
        float userLoudness = 1;
        public float Loudness
        {
            get => this.userLoudness;
            set => this.userLoudness = Check01(value);
        }
        ...
        public Glottis(int sampleRate, IRandomSource random)
        {
        ...
        void CalculateNewTenseness(float time)
        {
            ...
            // POC-FORK(7): jitter gated (upstream: ungated, gain fixed at 1).
            // Per-term gain keeps upstream float order — bit-identical at g=1.
            this.newTenseness = MathF.Max(0, this.TargetTenseness
                + this.tensenessJitterGain * 0.1f * this.NoiseGenerator.Simplex(time * 0.46f)
                + this.tensenessJitterGain * 0.05f * this.NoiseGenerator.Simplex(time * 0.36f));
        ...
        float CalculateVibrato(float time)
        {
            ...
            // POC-FORK(6): jitter gated (upstream: ungated, gain fixed at 1).
            // Gain multiplies each term (not a grouped sum) so g=1 keeps
            // upstream's float evaluation order — bit-identical parity.
            vibrato += this.pitchJitterGain * 0.02f * this.NoiseGenerator.Simplex(time * 4.07f);
            vibrato += this.pitchJitterGain * 0.04f * this.NoiseGenerator.Simplex(time * 2.15f);
            if (this.AutoWobble)
            {
        ...
        void SetupWaveform(float lambda)
        {
            ...
            this.waveformLoudness = MathF.Pow(Math.Max(0, tenseness), 0.25f); // POC-FORK(1)
        ...
        float NormalizedLFWaveform(float t)
        {
            ...
            return output * this.intensity * this.waveformLoudness * this.userLoudness; // POC-FORK(1)
        }
    }
}