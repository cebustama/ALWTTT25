...
> rendering POC (`reference/PinkTrombone_Rendering_POC_Verdict.md`, research
...
6. **Fork stays consumer-side.** `Assets/ThirdParty/PinkTrombone/` (~1000 lines,
   MIT, zero external dependencies, edits logged `// POC-FORK(n)` 1–7 in
   `FORK_NOTES.md`). It must never enter `MidiGenPlay.Runtime`. Audio-path
