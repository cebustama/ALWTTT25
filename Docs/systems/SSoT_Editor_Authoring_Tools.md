...
**Panel inputs:** optional `LLMClientData` override (when empty, the first `LLMClientData` asset in the project is used); free-text brief; card-kind hint; track-role hint; **intent seed** (seeds the deterministic palette pick — same payload + same seed ⇒ same palette; Randomize button); max-prompt-chars budget (pre-network cost cap; `0` = no cap; an over-budget prompt fails before anything is sent).
...
- The LLM fills **structured fields only**. Asset references are banned: any non-empty `cardSpritePath`, `trackAction.styleBundle`, or `modifierEffects` path entry — or a path/guid-shaped string smuggled into `modifierEffectNames` — is a hard failure listing every violation. Nothing is staged.
