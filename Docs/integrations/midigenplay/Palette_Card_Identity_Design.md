...
- ~~Author the §4 + §5 palettes as real assets~~ **Done:** 5 `DrumPatternData` +
  ...
  `Resources/ScriptableObjects/Patterns/Drums/` (patterns; per-palette sub-folders
  created in S5g: `FourOnTheFloor/`, `WaltzLilt/`, `OddMeterAngular/`,
  `CompoundSwing/`; the 5 original PCE patterns remain in `Patterns/Drums/New
  folder/` — optional rename to `PCE Base/` pending) and
  `Resources/ScriptableObjects/Drums/Palettes/` (palettes — unchanged).
