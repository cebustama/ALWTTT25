...
| ✅ `StatusAppliedEvent` | `StatusEffectContainer.Apply` (after `OnStatusApplied`) |  **WINK-1 (2026-08-31):** payload grew an additive, constructor-required `StatusEffectSO Effect` — the primitive id cannot identify which authored variant landed. Consumers must gate presentation on `DeltaStacks > 0` (a negative delta that leaves stacks standing also publishes). First bus event to carry **status-apply audio** (two clip layers, `SSoT_Audio` inv. 20) and a **persistent visual** (`StatusVisualDriver`, `SSoT_Status_Effects` §3.4). |
...
| ✅ `GigOutcomeEvent` | `GigManager.ResolveGigOutcomeAndEnd` + **`GigManager.LoseGig(GigLossCause)`** (TUT-REDESIGN-B) | **S4-shipped.** Carries Won; normal-flow only (editor Debug Win/Lose menus bypass by design). **TUT-REDESIGN-B (2026-09-03, D-TUTB-3=A):** payload grew a **`Cause` field (`GigLossCause`), mandatory in the constructor**; the Cohesion-collapse route now publishes (`cohesion_collapse`), closing the TLM-1 / tutorial blind spot. The parameterless `LoseGig()` stays mute on purpose (Dev/Debug, D-S4-SRC=A). |
...
| ✅ `CardPerformedEvent` | `HandController` (action commit) + `GigManager.TryPlayCompositionCard` (composition commit) | **WINK-1-shipped.** readonly struct. Payload: `Performer` (`MusicianBase`), `Card`. Published beside the existing `PlayCardOneShotAnimation` calls — i.e. **past every denial gate and before effects resolve**. `CardPlayedEvent` cannot serve this beat: it fires post-effects, which would put the performer's shout *after* the target's reaction. Adds no SFX key (card audio stays on `AudioType`, `SSoT_Audio` inv. 9/18). |
| ✅ `PlayDeniedEvent` | `GigManager.ReportPlayDenied` (single funnel) | **TUT-REDESIGN-B-shipped (2026-09-03, D-TUTB-1=A).** jugada rechazada; `PlayDenyReason` tipado + texto mostrado + carta + pagador. Publicado SÓLO desde `GigManager.ReportPlayDenied`, que también muestra el mensaje: un único sitio para "se lo dijimos al jugador" y "se lo dijimos al bus". Feeds: `HandController` (four action gates), `CompositionSession.Fail(msg, reason)`, the three `TryPlayCompositionCard` gates. Consumed by `TutorialController` (reason-routed: `tut_play_budget` / `tut_final_loop_lock` / `tut_harmony_denied`). Adds no SFX key. |
| ✅ `EarwormTickEvent` | `GigManager.AudienceTurnRoutine` (Earworm tick) | **TUT-REDESIGN-B-shipped.** un portador de Earworm procesado en el turno de audiencia; Vibe aplicado + stacks antes del decay. Drives `tut_earworm_tick`. Adds no SFX key. |
| ✅ `BonusLoopStartedEvent` | `CompositionSession` (bonus-loop start) | **TUT-REDESIGN-B-shipped.** el loop que va a sonar es bonus (R5-d); publicado antes del render. Drives `tut_bonus_loop`. Adds no SFX key. |
| ✅ `TrackReplacedEvent` | `SongCompositionUI` (same-musician, same-role replacement) | **TUT-REDESIGN-B-shipped.** una composición sustituyó otra del mismo músico y rol; no se publica para carriers sin bundle ni al repetir la misma carta. Drives `tut_track_replaced`. Adds no SFX key. |
...
| --- | --- | --- |
| ✅ `SensoryFxAdapter` (S2-new) | `AudienceReactionEvent` + `SongEndVibeEvent` + `AudienceVibeImpactEvent` (JUICE-PW) | **S2-shipped; Spawn since S3a.** Thin bus→FT adapter (D-S2-6=A). The S2 VerifyOnly mode is retained as a serialized fallback but the shipped mode is **Spawn**. JUICE-PW added the card-impact handler: staggered per-member FT + the procedural impact kick / particle burst on the landed target and on the performer (`FanoutIndex == 0`) —  **WINK-1 (2026-08-31)** added two handlers: `CardPerformedEvent` (performer floater + optional procedural burst/kick) and `StatusAppliedEvent` (status floater + soft impact kick on the receiver, resolved through `StatusEffectContainer.Owner`). Both honour the `VerifyOnly`/`Spawn` mode like every other handler. **TXT-3 (2026-09-10):** the status floater is a **player-text surface** and was retargeted — `SensoryFtPresentation.TryBuildStatusAppliedFt` now takes the name from the active `ConceptGlossarySO` by `StatusKey` (via `ConceptTooltipResolver`), not from `StatusEffectSO.displayName`, which is a developer label since that batch. **D-WINK-6=B is not reversed**: the text is still *derived* (name uppercased, no authorable field); only the registry it comes from changed. Colour still keys off `IsBuff` × owner side — gameplay semantics, not text — and the `DeltaStacks > 0` gate (ST-W7) is untouched. Found late, at batch close (**F-TXT-3-1**): it reads a *name*, not a tooltip, which is why a text migration nearly missed it. The **performer** floater still uses `CardDefinition.DisplayName`, one language slot — F-TXT-3-5, scope of TXT-4. |
...
| ✅ `SensoryAudioAdapter` (S3-audio-new) | `AudienceReactionEvent` + `SongEndVibeEvent` + `SfxStageCrossedEvent` + `RewardChoiceOpenedEvent` (S5h) + `AudienceVibeImpactEvent` (JUICE-PW) | **S3-audio-shipped.** Thin bus→audio adapter (D-SA-4=A); resolves each event to a `SensorySfxType` via `SensorySfxPresentation`, plays through `AudioManager` (the shared sink). Scene-placed on the gig Listeners object, mirroring `SensoryFxAdapter`.  **WINK-1 (2026-08-31)** added the `StatusAppliedEvent` handler — the first path here that plays a **clip directly** rather than resolving a `SensorySfxType`, because status audio is authored per-status/per-character and would otherwise need one enum key per status. |
