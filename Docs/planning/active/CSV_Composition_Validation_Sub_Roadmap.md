...
| **D-CSV-16** | Card → bundle reverse index (resolved CSV-4, 2026-07-20) | **A — CERRADA en CSV-4c (2026-08-10).** El índice inverso existe y está medido. Entregado como `CompositionInventoryWindow.Cards.cs` (partial): vista `Cards → Bundles`, columna `cards:` en Style Bundles, flags `UNREACHABLE` / `UNSOURCED` / `UNWIRED-SRC`, esquema de export nuevo. Superficie: `SSoT_Editor_Authoring_Tools.md` §17.10 / §17.13. Decisiones del lote: D-CSV4c-1=C (columna + vista) · D-CSV4c-2=B (las cuatro familias se escanean) · D-CSV4c-3=B (payload de Bassline) · D-CSV4c-4=B (`BandDeckData` es cuarta fuente) · D-CSV4c-5=B (tres flags, no uno). **La cifra de CSV-4 (14 de 33 progresiones alcanzables) queda histórica: se estableció de memoria y debe re-medirse con el export nuevo, no re-citarse.** |
...
| **CSV-4c** | Índice inverso carta→bundle (D-CSV-16=A) — vista `Cards → Bundles`, columna de alcanzabilidad en Style Bundles, flags `UNREACHABLE` / `UNSOURCED` / `UNWIRED-SRC`, columnas de payload de Bassline, octava vista en `Export All` | Editor tooling | `SSoT_Editor_Authoring_Tools.md` §17.6 / §17.8 / §17.10 / §17.13 | **cerrado 2026-08-10** — D-CSV-16 cerrada con herramienta; 8/8 smoke tests PASS; 18/35 bundles alcanzables, 17 `UNREACHABLE`. Hallazgos: aridad carta→bundle es 0..1; `BandDeckData` es cuarta fuente de descubrimiento; la cadena de Bassline es payload-born y termina en el bundle |
...
> El índice inverso carta→bundle (**D-CSV-16**) quedó **cerrado en CSV-4c** (2026-08-10). A partir
> de ahí, la alcanzabilidad de bundles y de progresiones es medible con herramienta. **Toda
> cifra de alcanzabilidad anterior a esa fecha —incluida la de 14 de 33 progresiones de esta
> misma sección— es histórica y debe re-medirse antes de reutilizarse.**
