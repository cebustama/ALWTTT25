...
| **SongHype threshold crossing (stage SFX)** | **`tut_first_sfx_stage`** | **S4** |
| **Neutral "sound cards" (tempo / key)** | **`tut_first_sound_card`** | **S4** |

10 trigger points reserved (8 original + 2 added by TUT-JAM-SEQ, 2026-06-16). Full design + draft text in `planning/active/Design_Tutorial_System_v0_1.md §6` and the jam sequence in **§6A**. Final dialogue text is S4 work; this matrix tracks coverage existence, not coverage quality (quality validated via S4 playtest).
...
**Modulation direction (MGP-ALWTTT-MOD-DIR-1).** Modulation cards in the current build land at a non-deterministic octave for the new root. `ModulationEffect` shifts the pitch class of `PartConfig.RootNote`; the octave is chosen by `ChordTrackComposer`'s voice leader, which minimizes voice-leading distance and routinely picks the descending neighbor for "up" modulations. A single Key Lift play may sound like an ascent, a settle, or a sidestep — interpreted as musical variety for the demo. Cross-project ask filed in MidiGenPlay tracker. *(Constrains beat 5 text — `tut_first_sound_card` must not promise an audible key direction.)*
