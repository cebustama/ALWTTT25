...
| **R8** | CONTENT / TEST **+ RUNTIME DE VOZ** ⚠ | **Alcance ampliado en R6 (2026-09-01): R8 ya NO es sólo contenido.** Arrastra (a) la **API one-shot de `SingerVoiceDirector`** para Singalong (D-R6-2=B: construirla sin consumidor era especulativo) y (b) **Tier B de Double Harmony** (D-R6-6: la armonía debe ser una segunda voz de Pink Trombone), que exige levantar `ActiveVoiceCap` a 2, **medir el DSP de dos voces** y cambiar la regla de selección del director, hoy filtrada a `Melody`/`Lead`. Planificarlo como «autorar unas cartas» es incorrecto. Además: rewards for all 4 (palettes via skills: jazz / Phrygian / jazz-vs-EDM drums; bossa v1 + tapping-or-degradation) + **Singalong** (on R6 one-shot API) + ~~starter v2 registration~~ **(hecho en TUT-REDESIGN-B, 2026-09-03, verificado por export: starter real 18/15, D-CAT-1..5 — `Design_Starter_Deck_v2_DRAFT.md` §3)** + full-band smokes (4 musicians, full pool) + campaign doc closure. **Nota (DOC-TUTR-B, 2026-09-04):** Super Slap (`r8_super_slap`) sigue en el pool de recompensa con coste Voltage 6 y **sin haber sido jugada nunca**. **Discrepancia registrada, no resuelta:** D2a=B (ledger R5-d) parqueaba `conito_super_slap` *sin* `UnlockedByDefault` hasta R8, pero el export de 2026-09-03 la lista en el pool (`IsReward ∧ UnlockedByDefault`) con id `r8_super_slap`. El asset es la verdad; decidir en R8 si el parqueo se restaura o la decisión se da por superada. | Last | R4–R7 |
