# Design_Tutorial_System_v0_3 — ALWTTT

> **v0_3 (2026-09-03/04, TUT-REDESIGN-A/B → DOC-TUTR-B).** Promoción de v0_2. Lo que cambia de
> verdad está en **§6C**: el arco de gig 1 se rediseñó para la **banda de 4** (Conito, Ziggy,
> Robot C2, Sibi — verificado contra el asset `DemoLaunchConfig` el 2026-09-03), con 11 beats
> guiados, 10 reactivos anclados al personaje, 34 ids y **cero reservados**. §6B se conserva
> como registro del arco de dos músicos (TUT-REBUILD) y **queda superado donde §6C lo
> contradice**; sus tablas no se reescriben. TUT-REFRESH (el "refresco" planificado tras R6)
> **queda superado** por TUT-REDESIGN-A/B: no se abrirá. El fichero se renombra v0_2 → v0_3
> (D-TUTR-6=B); `SSoT_INDEX.md` lleva la línea de sustitución.

**Status:** **Gig-1 arc for the band of 4 implemented and verified in-game (TUT-REDESIGN-B, closed 2026-09-03, 22/22 smokes ST-TUTB-1..21 green) — see §6C.** *Prior status, retained:* **S4 reactive system implemented (2026-06-17); superseded as the gig‑1 primary path by the guided curriculum (TUT‑REBUILD, 2026‑07‑08/09).** The reactive system is retained as a fallback and for post‑song‑1 coverage of the two beats with no guided equivalent (D3=B). Guided infra landed in **TUT‑R2 / R2b / R2c** (driver, input gates, scripted draw, holdLoop, D8 tokens, `MusicianStressHitEvent`, `AudienceBlockedEvent`, pulse highlights, `GigPresentationSO` telegraph toggles); **TUT‑R3** retires the 9 superseded reactive ids, runs the copy pass, and adds the highlight registry / world→screen pass. TUT‑R2 acceptance (CT1–CT6 + RT1–RT8 + ST1–ST12) was green; TUT‑R3 re‑runs ST11/ST12 after the retirement.
...
**Created:** 2026-05-23 · **v0_2:** 2026-07-09 (TUT‑REBUILD) · **v0_3:** 2026-09-04 (DOC-TUTR-B, doc-update of TUT-REDESIGN-B).
**Placement:** `Docs/planning/active/Design_Tutorial_System_v0_3.md` — renamed from
`Design_Tutorial_System_v0_2.md` 2026-09-04 (D-TUTR-6=B; same folder, same authority class).
v0_2 had been moved to `planning/active/` on 2026-08-08 (MANIFEST-1, D11=A) from `Docs/planning/`. Its predecessor
...
### Gig‑1 primary path = the guided curriculum (§6B → §6C)

> **v0_3 rider.** Since TUT-REDESIGN-B the gig-1 primary path is the **11-beat** arc for the band
> of 4 described in **§6C**. The "10-beat" and "two-musician" premises below are the TUT-REBUILD
> record and are superseded where §6C differs.
...
- On each candidate trigger: `firedDialogs.Contains(id)` → skip silently if true; else show + add. The controller consumes Sensory Event Bus events (`CardPlayedEvent`, `StatusAppliedEvent`, `LoopResolvedEvent`, `SfxStageCrossedEvent`, `SongEndVibeEvent`, `AudienceTurnStartedEvent`, `GigStartedEvent`, `GigOutcomeEvent`, `RewardChoiceOpenedEvent`, and — TUT‑R2 — `MusicianStressHitEvent`, `AudienceBlockedEvent`; and — **TUT-REDESIGN-B** — `PlayDeniedEvent`, `EarwormTickEvent`, `BonusLoopStartedEvent`, `TrackReplacedEvent`).

**Bus sources added by TUT-REDESIGN-B (2026-09-03).** All four are semantic (no SFX key); their producers are recorded in `Design_Sensory_Contract_v0_1.md` §3.

| Bus event | Fires | Beat(s) |
| --- | --- | --- |
| `PlayDeniedEvent` (`PlayDenyReason`, `DisplayText`, `Card`, `Payer`) | any denied play, from the single funnel `GigManager.ReportPlayDenied` — action path (`HandController`, four gates) and composition path (`CompositionSession.Fail(msg, reason)` + the three `TryPlayCompositionCard` gates) | routed by reason, table below |
| `EarwormTickEvent` | one Earworm carrier processed in the audience turn; Vibe applied + stacks **before** decay | `tut_earworm_tick` |
| `BonusLoopStartedEvent` | the loop about to sound is a bonus loop (R5-d); published before render | `tut_bonus_loop` |
| `TrackReplacedEvent` | a composition replaced another of the same musician + role; **not** published for bundle-less carriers nor when the same card is re-played | `tut_track_replaced` |

`PlayDenyReason` → beat (D-TUTB-4=A: `tut_play_budget` fires **only** on the ECON-1 denial):

| `PlayDenyReason` | Beat |
| --- | --- |
| ECON-1 budget | `tut_play_budget` (new, guided beat 7) |
| final-loop lock | `tut_final_loop_lock` |
| Harmony precondition (D-R6-4=A) | `tut_harmony_denied` |
| resource cost / inspiration / timing / bonus-loop precondition | no beat — message only (`SSoT_Card_System.md` §10.5) |

`GigOutcomeEvent` now carries `GigLossCause` (D-TUTB-3=A); **`tut_gig_lost` also covers the Cohesion-collapse defeat**, which before this batch published no event at all (`SSoT_Gig_Combat_Core.md` §16).
...
inspector assignment above. The `GameTextWindow` editor window (ALWTTT → Text → Game Text) reads
every catalog, builds one column per code, and edits `revisitTitle` + `pages` directly on the
`TutorialDialogSO` assets, with CSV export/import for a spreadsheet loop. The `.asset` files stay
...
The multi‑language authoring window half of D‑S5f‑3=B was **delivered in TXT‑1 (2026‑09‑05)** —
...
| 4 | ~~`tut_tracks_three`~~ **RETIRED (TUT-REDESIGN-B) → `tut_tracks_by_musician`, §6C** | after first composition | — | `song_panel_tracks` | Jam / 40 | "See the song panel? Three tracks: RHYTHM, BACKING and MELODY. Each composition card touches ONE of them. Drums set the pulse, chords build the world, melody is what sticks in your head." |
...
**Composure beat — gesto actualizado (R4, 2026-08-10).** Keep Cool pasa a exigir **arrastre sobre un músico** (`RequiresTargetSelection` derivado del payload tras el retarget `Self`→`Musician`, D-R0-3) y hereda el rechazo de objetivo aturdido. **El copy del beat no cambia; el gesto del jugador sí.** Verificado en ST-R4-9. **Fuera de este doc-update:** el realineamiento completo del tutorial con el roster actual (dos músicos nuevos, Conito→BASSLINE, melodía→Zig) y la reescritura de todos los diálogos son un lote propio — **TUT-REFRESH**, planificado después de R6 — no una edición de documentación. *(v0_3: TUT-REFRESH **superado** por TUT-REDESIGN-A/B, 2026-09-02/03; ver §6C.)*

### 6B.3 Driver / gate spec
- **Forced initial hand (F1).** Filled in `TutorialGuidedDriver.Awake` (`FillForcedInitialHand`, guarded by `!HasFired(YourTurn)`) — the order is Default Mode, Wormus Major, Singing Field, Warm Up, with role/domain fallbacks; a full miss hits the M4.5 guarantee (D1). *(v0_3: superseded — the forced hand is now Default Mode, Wormus Major, Finger Bass, Rise Up, four slots, §6C.)*
...
> Deuda cerrada en TUT-REDESIGN-B (2026-09-03): `tut_tracks_three` retirado; ver §6C.
...
## 6C. Gig‑1 arc for the band of 4 (TUT‑REDESIGN‑A/B, 2026‑09‑02/03)

**Premise.** Demo roster = **4 musicians: Conito, Ziggy, Robot C2, Sibi** — verified against the
`DemoLaunchConfig` asset on 2026‑09‑03. This replaces the two‑musician premise of §6B (and the
"roster = C2 + Sibi" statements that survived in `CURRENT_STATE.md` until DOC‑TUTR‑B). Design
lineage: TUT‑REDESIGN‑A (GAME DESIGN, 2026‑09‑02) produced the teachable‑mechanics inventory and
the redesigned flow (`TUT‑REDESIGN‑A_Inventario_y_Flujo.md`, chat deliverable, not in the PK);
TUT‑REDESIGN‑B (IMPLEMENTATION, 2026‑09‑03) built it. All code and editor wiring is applied and
verified in game: **22/22 smokes green (ST‑TUTB‑1..21)**; ST‑TUTB‑21 is verified **partially**
(see `coverage-matrix.md`).

### 6C.1 Guided arc — 11 beats (was 10)

`tut_jam_welcome` → `tut_your_turn` → `tut_play_composition` → **`tut_tracks_by_musician`** →
`tut_press_play` → `tut_loops_structure` → **`tut_play_budget`** (new, ECON‑1) →
`tut_inspiration_economy` → `tut_play_finisher` / `tut_play_finisher_early` → `tut_song_end_vibe`
→ `tut_audience_turn`.

- **`tut_tracks_by_musician`** replaces `tut_tracks_three`. Its copy counts no roles and does not
  name Harmony — with Double Harmony in Zig's starter there are five player‑visible roles, so any
  numbered claim would go false the moment the card is played (the §6B.3 debt, now closed).
- **`tut_play_budget`** (D‑TUTB‑4=A) fires **only** on the ECON‑1 denial (`PlayDeniedEvent`,
  reason = budget). It teaches the 1 Action + 1 Composition per musician per period rule at the
  moment the player hits it, not pre‑emptively.

### 6C.2 Forced hand and beat‑3 allow‑list

- **Forced hand (4 slots):** Default Mode, Wormus Major, Finger Bass, Rise Up — one composition
  per musician. The generics left the demo (the wired generic catalog is empty, D‑TUTB‑2 without
  practical effect), so the fifth slot of TUT‑REDESIGN‑A's draft is gone.
- **Slot 0 must be Default Mode:** the driver's `Awake` registers `forcedInitialHand[0].Id` as the
  `card_default_mode` highlight key and beat 3 highlights that card.
- **Fallbacks by role:** `{Rhythm, Backing, Bassline, Melody}`. The Action fallback disappeared
  with the fifth slot.
- **Beat‑3 allow‑list:** derived from the forced hand (TUT‑R2c), no code of its own. Today = the
  four compositions above.

### 6C.3 Reactive layer of gig 1

- The eight rewritten reactives of §6B.2 (`tut_status_buff_musician`, `tut_status_debuff_audience`,
  `tut_status_blocked_front`, `tut_musician_breakdown`, `tut_composure`, `tut_gig_won`,
  `tut_gig_lost` — now also covering the Cohesion defeat — plus the retained `tut_first_*`) **plus**
  `tut_final_loop_lock` and `tut_track_replaced` (bus sources in §3.1).
- **Character‑anchored reactives (10):** `tut_earworm_tick`, `tut_captivated`,
  `tut_voltage_first`, `tut_overload_ready`, `tut_bonus_loop`, `tut_spotlight`,
  `tut_read_the_room`, `tut_harmony_track`, `tut_harmony_denied`, `tut_sung_melody`.

**Suppression rule (D‑TUTB‑6=B).** The character‑anchored reactives stay silent while the guided
arc is pending and are released as soon as `tut_audience_turn` fires — i.e. in **song 2 of the
same gig**. This is deliberately **not** per‑gig: the demo has a single gig, so per‑gig scoping
would have made these beats unreachable. Implementation:
`TutorialGuidedDriver.ApplySuppressionForArcState()`, the single call point invoked from
`OnEnable`, `PrepareForGig`, `OnAudienceTurn` and `OnSongEnd`. (D‑TUTB‑5 is superseded by this.)

**Routing by `StatusKey`, never by card id** (rider of D‑TUTR‑1): `flow` → the generic musician‑buff
beat; `voltage` → `tut_voltage_first` (+ `tut_overload_ready` when the serialized threshold is
reached — `TutorialController.voltageOverloadThreshold`, today **3**); `spotlight` → its own beat;
`captivated` → its own beat; any other musician buff → no beat, log only. Why not card id: a
status is what the player *sees*; two cards can apply the same status and a future card must not
need a tutorial edit to be taught.

### 6C.4 Ids and parity

**Ids: 20 → 34. `ReservedUnauthored` is empty.** `tut_flow` was absorbed by
`tut_status_buff_musician` (Flow‑keyed) and `tut_audience_preferences` by `tut_read_the_room`.
The parity guard (`ALWTTT/Tutorial/Validate catalog language parity`) compares the 34 canonical
`TutorialTriggerId` constants against each catalog; EN and ES pass. The new seeders are
`Author/Seed TUT‑REDESIGN‑B dialogs EN/ES (13, band of 4)` on `TutorialDialogCatalogSO`; the
`tut_tracks_three` string survives in the catalog only as a deliberate literal in the retirement
list (the constant no longer exists).

**Supersession ledger.** The one in `TUT‑REDESIGN‑A_Inventario_y_Flujo.md` §4, with one
correction: the forced hand uses **4 slots, not 5** (generics left the demo).

### 6C.5 Declared gap and open findings

- **O5 — `tut_read_the_room` is authored (EN + ES, parity green) but has no wired trigger.** What
  `crd_sibi_read_the_room` does in code has not been verified; the beat exists as text only.
- **F‑TUTB‑4 — `status_icon_musician` resolves to the last registered target.** With 4 musicians
  the Voltage / Spotlight beats can spotlight the wrong musician. Fix identified, not applied:
  `ReregisterKeyOn` inside `OnStatusApplied` (the §5.3 D3=B precedent for `MusicianStressHit`).
- **F‑TUTB‑5 — the tutorial modal covers the panel it is highlighting.** Presentation.
- **F‑TUTB‑1 — `initialGigInspiration = 0` in the asset vs 1 documented** (§8.2 / §9.1 say
  D‑TUT‑R2‑1=B "confirmed applied"). Which is the truth is **open**; this doc does not decide it.
- **F‑TUTB‑3 — a `PlayerTurn` branch of `GigManager` (~l.1012) calls `WinGig/LoseGig` without
  publishing `GigOutcomeEvent`.** Reachability unverified.
- Songs 3–6 of a gig have never been played: deck recycling at that depth is code truth, not
  observed behaviour (D‑TUTB‑7=C raised the song ceiling to 6, `SSoT_Gig_Combat_Core.md` §16).

Findings are recorded here and in `CURRENT_STATE.md` §4; none is closed by this doc.

### 6C.6 Decision ledger (TUT‑REDESIGN‑B, 2026‑09‑03)

D‑TUTB‑1=A (`PlayDeniedEvent` from the single funnel `GigManager.ReportPlayDenied`) ·
D‑TUTB‑2=A (generics disabled by `flags=None`; no practical effect — the wired generic catalog was
already empty) · D‑TUTB‑3=A (`GigLossCause` in `GigOutcomeEvent`; `LoseGig(GigLossCause)` for the
Cohesion route) · D‑TUTB‑4=A (`tut_play_budget` only on the ECON‑1 denial) · D‑TUTB‑5=A
(**superseded by D‑TUTB‑6**) · D‑TUTB‑6=B (suppression released on guided‑arc completion, not on
gig change) · D‑TUTB‑7=C (song ceiling 6 + early victory when the whole audience is convinced).
Catalog decisions D‑CAT‑1..5 are starter‑deck decisions and live in
`Design_Starter_Deck_v2_DRAFT.md` §3 + `CURRENT_STATE.md`.
...
- **`Keep Cool` catalog placement — RESOLVED (T3a):** authored as a **C2-owned** card (`FixedMusicianType`), not generic; C2 is always in the demo band (roster = C2 + Sibi *— v0_3: the demo roster is the band of 4, §6C; C2 is still always in it*). The generic-catalog move is **deferred pending D-ECON-GENERIC** (who spends the ECON-1 per-musician action budget for an `AnyMusician` card — home `Design_Action_Economy_v1` / `SSoT_Gig_Combat_Core §14`).
...
- ~~**Reserved ids** `tut_audience_preferences` (D6) / `tut_flow` (D‑TUT‑R1‑4) remain reserved without a trigger.~~ **CLOSED (TUT-REDESIGN-B, 2026-09-03):** both absorbed (§6C.4); `ReservedUnauthored` is empty. Note `tut_read_the_room` inherits the *trigger* gap (O5).
...
**TUT‑REDESIGN‑B (2026‑09‑03):** D‑TUTB‑1..7 — see §6C.6. **D‑TUTR‑6=B** (v0_2 → v0_3 by rename, not a new document).
...
- `coverage-matrix.md` (tutorial row → guided primary; doc home `Design_Tutorial_System_v0_3` since DOC-TUTR-B).
- `PENDING_DOC_DIFFS_TUT-REDESIGN-B.md` (consumed by DOC-TUTR-B 2026-09-04; retired from the PK) · `TUT-REDESIGN-A_Inventario_y_Flujo.md` (chat deliverable, supersession ledger §4).
- `TUT-REBUILD_Sub_Roadmap.md` (TUT‑R* arc home).
- `SSoT_Gig_Combat_Core.md` §16 (gig end: `LoseGig(GigLossCause)`, early victory, song ceiling) · `SSoT_Card_System.md` §10.5 (denials with message).
