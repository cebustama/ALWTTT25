...
1. **Phase B B3 — Content + design (expanded 2026-05-15).** Aditivo. Inspiration cost/gen balance pass (#9), BPM cards (#10), Modulation cards (#11), Sibi musical identity via per-card `InstrumentEffect` on Singing Field (#11.5, β path), **audience pool authoring (#12 expanded to 2 archetypes):** Cool Dude → 3 abilities (Move One Step parameterized + Heckle with `Exposed` + Stress + Indifference self-buff implementing new `ApplyIncomingVibe` helper); Kid → 2 abilities (existing band-wide Stress + new `Egged On` buff on Cool Dude's outgoing Stress); demo encounter (2×Kid + 1×CoolDude) authored as `GigEncounterSO`. Plus B3 candidate slate from B2.5 (C/D authoring/inspector polish, E-lite Blocked tooltip, F real `ResolveLoopEffect` impl, G filter draws during composition session per D-B3-DrawFilter=B, H Always-action discard semantics, I ParentActive=False warning) and design gaps #12-15 inherited. See `Roadmap_ALWTTT.md §5.3`.

2. **Demo cut prep (new batch, opened 2026-05-15).** Mini-batch between B3 close and §5.4. Quick-start flag on `GigDevSettingsSO` (DC-1=C, D-DCP-1=A), `DemoLaunchConfigSO` (DC-3 demo structure: 4 songs × 1 part × 4 loops, Inspiration 3/1 moderate per DC-4=B), new SFX→FlatVibe bonus mechanic (post-Flow flat addition, scaled 3/6/10 defaults per D-DCP-2=A, tunable on `GigPresentationSO`), tuning pass on demo encounter (target 60-80% win rate), `Design_Demo_Cut_v1.md` coverage matrix shipped. See `Roadmap_ALWTTT.md §5.3.5`.

3. **Demo readiness review (post-Demo cut prep).** Confirms Phase B exit and sets the publisher/community demo cut.

4. **Pitch deck refresh (new stream, opened 2026-05-15).** Non-governance, non-code. Sub-batch A (audit + outline + draft text) can begin informally pre-§5.4 per PD-1=C interpretation α; sub-batches B-C run after §5.4. Replaces obsolete `GoblinzStudio.pdf` (Aug 2025). Target PD-3=C (deck + video + playable build), minimum PD-3=B (deck + video). PD-4 commercial info captured in roadmap §6. See `Roadmap_ALWTTT.md §6`.

5. **Post-demo follow-ups (parked).** F-2 D4 (`MaxInspiration` / `MaxCardsOnHand` to `GigFlowSettingsSO`), MB5 (action-card cost gate on `CanPlayActionCard`), `TryPlayCompositionCard` step 8 mirror, `SSoT_Gig_Combat_Core.md §4.2` one-line dual-siting note, F-4 Stage B (reopens automatically if `[F-4][MMM]` LogError fires during playtest), MidiGenPlay package-internal Player-build errors (`MidiGenPlayConfig.GetChordWriteFolder` / `GetProfileForTonality` — separate MidiGenPlay-project batch; **blocks PD-3=C build packaging** if unresolved), Audience Member Wizard Editor, Action-card playability-during-loop gate verification (see §4 watch-item), B2.5 content-dependent items #4 (per-venue smoke/fire), #5 (CompositionFxConfigSO tuning), #6 (animation feel).
...
- **Demo cut prep batch (opened 2026-05-15).** Mini-batch between B3 close and §5.4 Demo readiness review. New mechanic surfaced: SFX→FlatVibe bonus on SongHype threshold crossings (currently visual-only per B2 #6). Three tunable floats on `GigPresentationSO` (`sfxBonusVibeStage1/2/3`, defaults 3/6/10 per D-DCP-2=A, post-Flow flat addition not Flow-scalable). Quick-start path: new `autoStartFromDefaults: bool` + `demoLaunchConfig` reference on `GigDevSettingsSO` (D-DCP-1=A); `GigSetupController.Start()` auto-invokes `OnStartPressed` when both set. `DemoLaunchConfigSO` (new SO) bakes: C2+Sibi roster, demo encounter, requiredSongCount=4, JamRules 1×4, initialGigInspiration=3, inspirationPerLoop=1. Tuning + validation pass (8-10 playthroughs, target 60-80% win rate). `Design_Demo_Cut_v1.md` shipped at closure with effect/modifier/status coverage matrix. See `Roadmap_ALWTTT.md §5.3.5`.
