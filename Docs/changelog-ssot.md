...
and restore invariant 7 (operational focus stays brief). §1's per-batch closed-work log (which
duplicated this changelog) collapsed to a Foundation summary + a current-milestone closure table;
the [GAP — UNVERIFIED] ALWTTT-PCE-PROP stub preserved. §2 trimmed (closed entries removed); §5
replaced (doc-closure history → forward-looking); §3 sequence-status + S4 row refreshed (S4 now
next-active, inheriting the TUT-JAM-SEQ design). §3 tables + §4 (open items + risks) + §6 unchanged.
Pre-prune full version archived as Docs/archive/CURRENT_STATE_pre-prune_2026-06-16.md (snapshot,
historical-only, §15.3) — no history lost. changelog-ssot.md left whole (DOC-HYGIENE D1=A).
No code, no authority move, no live doc renamed/moved → no coverage-matrix / SSoT_INDEX edit.
Classification: structural + operational. No SSoT / contract change.

2026-06-16 — TUT-JAM-SEQ: GUIDED JAM TUTORIAL SEQUENCE (DESIGN + DOC, no code)

Design batch (GAME DESIGN). Specifies a 6-beat guided tutorial sequence over the demo's first song
for the composition ("jam") system, which a design pass found under-covered (2 isolated popups).
Doc-only; no runtime change. Implementation deferred to S4 (next-active per CURRENT_STATE §1).

Decisions resolved:


D-TUT-6=A — jam gets its own guided sequence over the 1st song (scoped exception to D-TUT-1
"basics only"; the jam is the core mechanic, layered; not license to inflate coverage).
D-TUT-7=C — no rich illustrated codex now; the D-TUT-2 revisit menu is "codex-lite"; rich codex
deferred to Phase C / post-demo.
D-TUT-8=A — sequence runs live on the 1st real song, fired by Sensory Event Bus events; beat 5
(tempo/key) is opportunistic.
D-TUT-9=A (O1) — 3 of 8 existing demo triggers fold in (tut_first_composition_card,
tut_first_inspiration_spend, tut_first_song_end → beats 1/2/6); 5 stay standalone (welcome,
action card, audience action, status applied, gig won). Action cards + audience pressure are
combat-side, not jam.
D-TUT-10=A (O2) — independent first-time triggers in natural gameplay order (reuse the D-TUT-3
firedDialogs HashSet); NOT an ordered step-state machine. Adds one minimal controller requirement:
a single-modal queue (no stacking; same-resolution multi-fire shown in authored order).
D-TUT-11=A (O3) — revisit menu stays already-fired-only (no read-ahead).


New triggers (2 missing coverages + 1 new beat):


tut_first_sfx_stage (beat 4) — first SfxStageCrossedEvent (SHIPPED S3-audio); SongHype threshold
crossing → stage SFX escalation.
tut_first_sound_card (beat 5, opportunistic) — first tempo/modulation composition card
(Push It / Key Lift); "sound cards reshape the music, not your meters."
tut_first_loop_inspiration (beat 3) — first per-loop inspiration gain; "tracks feed Inspiration
back each loop."
...
The rehydration's "S2 infra / S2-blocked / S2 tutorial-overlay batch" framing was stale. S2 closed
2026-06-14 (bus shipped); TutorialController was deferred S2→S4 (D-S2-5). The controller, R1/R2
overlay, bus-event extension, and authoring all land in S4 (next-active per CURRENT_STATE §1).
Design_Tutorial_System status line, §3 ordering note, and §9 deferrals corrected from S2 to S4.
The jam beats mostly consume bus events that are NOT shipped: only AudienceReactionEvent,
SongEndVibeEvent, SfxStageCrossedEvent exist. Beats 1/2/3/5 need CardPlayedEvent + an
Inspiration-delta signal + a per-loop-gain signal with no current inventory entry. S4 = bus
extension + overlay + authoring, not authoring-against-existing-events.


Doc changes (this batch):


Design_Tutorial_System_v0_1.md — new §6A (jam sequence: 6 beats, O1 reconciliation, 3 new IDs,
D-TUT-6..11 ledger); §6 pointer line; §2 D-TUT-6 exception note; §4 single-modal-queue +
§5 already-fired-only paragraphs; §8 DoD items (queue, opportunistic beat 5, bus-events-to-add);
status line / §3 ordering note / §9 corrected S2→S4; §10 cross-refs updated.
Design_Demo_Cut_v1.md §2.4 — +2 coverage rows (SongHype threshold → tut_first_sfx_stage; neutral
sound cards → tut_first_sound_card); jam-sequence note; MGP-ALWTTT-MOD-DIR-1 limitation annotated
with the beat-5 text constraint.
Classification: semantic (new teaching coverage) + lifecycle (S4 plan inheritance). Planning-only —
no SSoT / authority / contract change (both docs are reference(planning)); SSoT_INDEX,
coverage-matrix, and CURRENT_STATE §3 unaffected.


No smoke tests: design+doc only, no runtime/gameplay change. S4 owes tutorial-trigger smoke tests
(ST-S4-1..11 + queue / persist / reset / revisit / opportunistic-beat-5 / no-direction) per
Sensory Contract §7.

2026-06-16 — AUDIO-CHAR-PROFILES-2 (PHASE 2): PER-ABILITY SFX

Added:
- `AudienceAbilityData.abilitySfx` (`AudioClip`) + `AbilitySfx` getter (in `AudienceCharacterData.cs`) — inline per-ability one-shot, beside the `AbilityAnimationData` trigger. Additive serialized field; existing audience assets default it to null (no serialization break).

Changed:
- `AudienceCharacterBase.AbilityRoutine` — fires `AudioManager.Instance?.PlayOneShot(NextAbility.AbilitySfx, jitter: false)` once at ability activation, at the animator-trigger site, after the stun/null/empty guards and before `PlayAbilityAnimation`. Single-source → immediate (jitter stays fan-out-only); independent of the animation guard; null clip no-ops in the sink.
- `CharacterSfxProfileSO` class-summary forward-note corrected: ability SFX is NOT hosted on the profile (D-ABILITY-SFX-HOME=(i)); the SO stays reaction-only. Comment-only.

Decisions resolved: D-CHAR-SFX-2=A (ability-level fire; option B status-apply deferred, not rejected); D-ABILITY-SFX-HOME=(i) (inline on `AudienceAbilityData`, not a keyed profile map — `abilityName` is a display string, not a stable key). `MusicianCharacterData` deliberately gets no `sfxProfile`: ability fire is audience-only; musicians use card-direct `AudioActionType`.

Semantic / structural changes:
- Per-ability SFX is profile/data-sourced — **no new `SensorySfxType`** (invariant 2 intact); `AudioManager` stays the dumb sink (reuses the phase-1 `PlayOneShot(AudioClip, jitter)` seam). SSoT_Audio §3 (new per-ability paragraph) + invariant 17. Fire is immediate/single-source (invariant 10 unchanged); null → no-op (invariant 3). coverage-matrix +1 per-ability row.
- `CharacterSfxProfileSO` unchanged (reaction-only); `AudienceCharacterBase.cs` stays under the Audience SSoT (not added to SSoT_Audio governs — same precedent as the GigManager ambience hooks).

Governance backfill (phase-1 misses, folded into this close): `ssot_manifest.yaml` SSoT_Audio — added invariant 16 (per-character reaction SFX, omitted at the phase-1 close) and invariant 17 (ability SFX), and added `CharacterSfxProfileSO.cs` to `governs` (Assets path flagged `# M1-note:` for verification). `CURRENT_STATE.md` — phase 1 closed without a §1 block; the new AUDIO-CHAR-PROFILES-2 block supersedes the stale "Next-active: AUDIO-CHAR-PROFILES" pointer and records phase-1 closure.

Smoke: ST-ABIL-1..6 PASS (correct per-character clip at activation; clean no-op when unauthored; immediate-not-jittered; fires without an animation block; reaction-path regression ST-CHAR-1..7 intact). ST-ABIL-5 (stunned ability = silent) deferred to Dev Mode / M1.5 — Stun not player-applicable; fire-site ordering correct by construction.

Migration impact: existing abilities keep working with no clip (silent; null → no-op). Authoring an `abilitySfx` on an ability opts it into a per-activation sound. Status-apply SFX (option B) remains a deferred parallel hook.

2026-06-16 — AUDIO-CHAR-PROFILES (PHASE 1): PER-CHARACTER REACTION SFX

Added:
- `CharacterSfxProfileSO.cs` (Data) — per-character SFX clip source. Pos/neg `List<AudioClip>` (mirrors `SoundBankSO.SensorySoundEntry` clip-list + `RandomItem()` shape) + `GetClipFor(SensorySfxType)` returning a random clip for `ReactionPositive/Negative`, null otherwise. Reaction-only in phase 1; ability/status map is an additive phase-2 field.
- `AudioManager.PlayOneShot(AudioClip clip, bool jitter)` — overload that plays a pre-resolved clip through the existing private jitter coroutine (immediate when `sfxMaxJitterSeconds <= 0`). The sink stays dumb and learns nothing about characters.
- `AudienceCharacterData.sfxProfile` (+ `SfxProfile` getter) — assignment point for the per-character profile.

Changed:
- `SensoryAudioAdapter.OnAudienceReaction` — resolves polarity, then the reacting character's `sfxProfile` clip *source* first; falls back to the global `SoundBankSO` **per polarity** when the profile has nothing for that polarity. `jitter: true` preserved on both paths. Neutral still returns null → intentionally silent (unchanged). Class-summary forward-ref updated to the realized behavior.

Decisions resolved: D-CHAR-SFX=C (phased); D-CHAR-SFX-SHAPE (SO, pos/neg only, neutral FT-only, field `sfxProfile`); D-CHAR-SFX-FALLBACK=per-polarity; D-CHAR-SFX-SEAM (adapter-side resolve + AudioClip+jitter overload). Open → phase 2: D-CHAR-SFX-2 (ability-level vs status-apply fire).

Semantic / structural changes:
- Per-character reaction clips are an alternative *source* for the existing `ReactionPositive/Negative` keys — **no new `SensorySfxType`** (invariant 2 clarified). SSoT_Audio §3 (new per-character paragraph) + §2 ownership row + invariant 16. Resolution order: per-character profile → bank per polarity → warn-once/no-op (invariant 3).
- `MusicianCharacterData` intentionally unchanged this phase (musicians don't emit reactions; its existing `profile` is the unrelated melodic-leading config). Musician `sfxProfile` lands in phase 2.
- Per-character coverage is not audited; the `SoundBankSO` "Audit SFX Coverage" surface remains the net. coverage-matrix +1 row.

Smoke: ST-CHAR-1..7 PASS — distinct per-character pos/neg (profile path); clean bank fallback for a profile-less character; per-polarity fallback on a half-authored profile; neutral-silent regression (FT-only rule); jitter fan-out regression (invariant 10); single-source-cues-unchanged regression (song-end/stage-cross stay immediate + bank). No deferrals.

Migration impact: existing audience assets keep working with no profile (bank fallback). Authoring a `CharacterSfxProfileSO` and assigning it to a character's `sfxProfile` opts that character into distinct reaction sounds. Phase 1 is reaction-only; ability/status SFX (AUDIO-CHAR-PROFILES-2) are a fast-follow.

2026-06-16 — AUDIO-AMBIENCE: LOOPING CROWD AMBIENCE (SFX GROUP)

Added:
- `AudioManager` ambience bus (SFX group): self-provisioned looping `ambienceSource` + fade API `FadeInAmbience`/`FadeOutAmbience`/`SetAmbienceLevel01`/`StopAmbience` + `IsAmbiencePlaying`. Effective = masterSfx × ambienceLevel × fade. Serialized `ambienceClip` / `ambienceLevel01` / `ambienceFadeInSeconds` (1.2) / `ambienceFadeOutSeconds` (0.6). Null clip → warn-once + no-op.

Changed:
- `AudioManager.SetSfxVolume01` caches the master-SFX level and recomputes ambience — the SFX slider scales ambience alongside sfxSource + buttonSource.
- `GigManager` — four lifecycle hooks: StartGig (FadeIn / crowd present), OnPlayPressed (FadeOut / duck), OnCompositionSessionEnded (FadeIn / return, guarded by wasPlaying), OnDestroy (StopAmbience / no scene bleed). Duck/return are volume-only (loop never restarts).

Decisions resolved: D-AMB-BUS=A, D-AMB-HOME=A, D-AMB-FADE=B, D-AMB-HOOK=A, D-AMB-CLIP=A.

Semantic / structural changes:
- New ambience bus under the SFX group; SSoT_Audio §3 (ambience subsection) + invariant 15.
- Return hooks the synchronous song-end chokepoint (OnCompositionSessionEnded nulls _session inside Tick → GigManager.Update bypasses the state-transition block); polling _isSongPlaying cannot catch the end edge.
- GigManager.cs stays under its gig SSoT (not added to SSoT_Audio governs — same precedent as the AUDIO-OST hook); manifest SSoT_Audio +invariant 15; coverage-matrix +1 row.

Smoke: ST-AMB-1..8 PASS (incl. ST-AMB-4 cross-part no-flicker + ST-AMB-6 SFX one-shot regression). No deferrals.

Migration impact: ambience is a first-class SFX-group surface on AudioManager; gig-scoped (started at gig open, stopped on teardown). Per-venue beds are a future AmbienceCatalogSO (not SoundBankSO).

2026-06-16 — AUDIO-OST: OST MUSIC PLAYBACK

Added:
- `OstTrackId.cs` (Enums) — OST track key (None=stop sentinel, MainMenu).
- `OstCatalogSO.cs` (Data/Audio) — OST inventory id→clip+loop+defaultLevel01; scene→track map lives on the director (inventory-vs-caller split, mirrors SoundBankSO).
- `MusicDirector.cs` (Managers) — DontDestroyOnLoad singleton owning OST; two owned AudioSources → real crossfade; Play/Stop/CrossfadeTo/RefreshMusicLevel; scene reaction via SceneManager.sceneLoaded + serialized build-index map (unlisted scene → None/stop). No AudioMixer — OST volume is AudioSource.volume = musicLevel01 * defaultLevel01.

Changed:
- `GigManager.DevSetGlobalMusicVolume01` also calls `MusicDirector.Instance?.RefreshMusicLevel()` — the Music level now scales gig music AND OST (one level, two consumers).
- Optional one-home cleanup: dead `AudioManager.PlayMusic` + `musicSource` (zero callers) retired.

Decisions resolved: D-OST-HOME=B, D1=A, D2=A, D3=A, D4=A, D-OST-DOCS-1=A.

Semantic / structural changes:
- New OST playback bus under the Music group; SSoT_Audio §4.5 + invariants 12–14.
- SSoT_Audio now governs MusicDirector.cs, OstCatalogSO.cs, OstTrackId.cs (manifest governs: +3, +3 invariants; coverage-matrix +1 concept row).
- OST asset pipeline recorded as a convention (SSoT_Audio §4.5): WAV source → Unity Vorbis import (no pre-convert), loudness via Music level + defaultLevel01.

Smoke: ST-OST-1..7 PASS. Deferred: ST-OST-8 (true crossfade — single track), live-Music-slider-on-playing-OST (Dev tab gig-only / OST menu-only).

Migration impact: OST is a first-class Music-group surface alongside gig music; gig and OST never play simultaneously (gig scenes unlisted → OST stops).

2026-06-15 — AUDIO-SFX-FIX: SFX CORRECTNESS + SCOPED JITTER

Changed:
- Card SFX is opt-in by type: `AudioActionType.None` appended; `CardBase.Use()` skips `Button`/`None` so un-tagged cards are silent (fixes heal-sound-on-every-card + on the Main Menu Start button). Append, not reorder → existing serialized AudioType ints unchanged.
- Master SFX level applied app-wide at `AudioManager.Awake` (D-SFX-APPLY=A); governs `sfxSource` + `buttonSource` (UI clicks now under the SFX slider).
- SFX jitter made caller-controlled (D-SFX-JITTER-SCOPE=B): `PlayOneShot(SensorySfxType, jitter)`; only `SensoryAudioAdapter`'s audience-reaction fan-out opts in. `sfxMaxJitterSeconds` on AudioManager (default 0.15, 0 = off).

Decisions resolved: D-SFXDEF=B, D-SFX-APPLY=A, D-SFX-JITTER-SCOPE=B, D-SFX-JITTER-HOME (field).

Content: `ButtonSoundProfile` re-keyed off `CardSFX-HealStress`; Draw card re-tagged off `Button`.

Smoke: ST-SFX-1..8 PASS.

Migration impact: the SFX subsystem (SSoT_Audio §3) now documents opt-in-by-type, caller-controlled jitter, and app-wide SFX with the UI bus included.

2026-06-15 — M-AUDIO-MIX: AUDIO MIXING TOOLING + SSoT_Audio

Added:
- `systems/SSoT_Audio.md` — governed audio home (SFX subsystem migrated from Sensory Contract §5A + music-mix model + audio boundary).
- `planning/active/Roadmap_Audio.md` — audio work-stream sub-roadmap (planning-only).
- `AudioMixSettingsSO` (global music + master SFX + per-musician list); Dev "Audio Mix" tab; GigManager mix load/re-apply + `#if ALWTTT_DEV` audio wrappers; AudioManager `SetSfxVolume01`/`SfxVolume01`; MidiMusicManager song-start channel re-assert.

Decisions resolved: D-VOL=B, D-AUDIO-SSOT=B, D-MIX-HOME=B, D-MIX-FALLBACK=B, D-AUDIO-MANIFEST=yes. Boundary: MIDIInstrumentSO.volume01 not wired (instrument01 == 1.0).

Semantic / authority changes:
- Global music volume migrated from `GameplayData.globalMusicVolume01` (deprecated, removed) to `AudioMixSettingsSO` — one home per audio concept.
- SFX subsystem authority moved from Sensory Contract §5A (planning) to SSoT_Audio §3 (governed).
- `MidiMusicManager` mix-axis overlap with the CompositionSession integration SSoT declared as manifest finding F8 (expected overlap, not drift).

Migration impact: `AudioManager.cs` is now governed (was ungoverned). Audio is a first-class subsystem with an SSoT, a manifest entry, and a planning sub-roadmap.

Smoke: ST-AM-1..7 PASS.

2026-06-14 — S3-audio AUDIO SFX LAYER — CLOSED (placeholder clips)

Closed; closes the S3 umbrella. Wires the audio half of Standing Directive #2 for card-play and
the three bus surfaces, end to end, with placeholder clips. The bus is now the sole sensory
fan-out for FT and audio (independent handlers). Audio is the FT→SFX upgrade; the FT floor (D2)
was already met everywhere.

**Finding that reframed the batch.** Audio was not greenfield: `AudioActionType` is serialized on
`CardDefinition` and exposed to the LLM card pipeline, and `AudioManager.PlayOneShot(...)` was
already called (unguarded) in `AddStressAction`/`BlockStressAction`. Neither type was in the
initial shared file set. D-SA-1 resolved to flesh-out, not greenfield.

**Decisions.** D-SA-1=A (flesh out the existing `AudioManager`); D-SA-2=A (infra-then-assets;
placeholder tones rejected per D1); D-SA-3 (event→`SensorySfxType` mapper); D-SA-4=A (one
`AudioManager` sink, two callers — card-direct + bus); D-SA-5=A (`SfxStageCrossedEvent`, published
after VFX+bonus; VFX stays direct, D-S3-6=A); D-SA-6=A (separate `SensorySfxType`, not an
`AudioActionType` extension — which is card-authored + LLM-coupled in three sites); D-SA-7=B
(`SoundBankSO` central inventory + `Audit SFX Coverage`).

**Code.** New `SoundBankSO.cs`, `SensorySfxType.cs`, `SensorySfxPresentation.cs`,
`SensoryAudioAdapter.cs`, `SfxStageCrossedEvent.cs`. `AudioManager.cs` — loads `SoundBankSO`;
card + sensory dicts; `PlayOneShot(SensorySfxType)` overload; null-safety fix (warn-once + return;
was a live NRE on an unassigned profile) + warn-once-per-type. `GigManager.FireSongHypeStage` —
publishes `SfxStageCrossedEvent`. `CardBase.Use()` — card-play audio via
`AudioManager.PlayOneShot(CardDefinition.AudioType)` (canonical card path; corrected from the
mis-targeted `AddVibeAction`, which was reverted; redundant card-context calls recommended for
removal from `AddStress`/`BlockStressAction`, audience-context branches kept).

**Smoke tests.** ST-SA-1..9 PASS (infra; ST-SA-6 stage-publish + ST-SA-9 card-audio-layer fixed
mid-batch). ST-SA-A1..A4 PASS (audible: card / reaction / song-end / stage-crossing, placeholder
clips). Coverage audit caught a `SoundProfileData` mis-keyed internally ("Heal Vibe SFX" not set
to `HealVibe`); fixed.

**Placeholders / D1.** Clips are placeholders — sound exists (so §7 is satisfied; not "sound
later"), but final intentional non-generic SFX is a tracked D1 follow-up.

**Follow-ups queued.** Music-mix batch (D-VOL=B: Dev "Audio Mix" tab + persisted per-musician /
global balance — fixes "music sometimes too loud"; ALWTTT-side via `MidiMusicManager.
SetMusicianVolume01`; `MIDIInstrumentSO.volume01` is MidiGenPlay-side and not wired; runs before
S4), which also creates `SSoT_Audio.md` (D-AUDIO-SSOT=B, consolidating §5A + the music-mix model
+ the audio boundary). Plus: final SFX (D1); smoke/fire appear/loop animation; stale
`LoopScoreCalculator.cs` refresh + `LoopScoringConfig`/`HypeThresholds`.

**Docs.** `Design_Sensory_Contract_v0_1.md` §3 (SfxStageCrossedEvent shipped; SensoryAudioAdapter
+ SensorySfxPresentation consumers) + §4 (audio column → shipped/placeholder; S3-audio note) +
new §5A (audio subsystem as-built). `CURRENT_STATE.md` §1 closure block + §3 (S3 → S3a + S3-audio,
both closed) + sequence-status (music-mix next, then S4) + §4 open items. This entry.
`ssot_manifest.yaml`, `coverage-matrix.md`, `SSoT_INDEX.md`, `SSoT_Card_Authoring_Contracts.md`,
`SSoT_Card_System.md`, `Roadmap_ALWTTT.md`, other SSoTs intentionally unchanged (audio subsystem
standalone like the bus; `audioType` already a documented card field; no authority/contract change).

2026-06-14 — S3a SENSORY POLISH (VISUAL)

Closed. S3a completes the S2 migration and lands the visual sensory layer; the
audio half of Standing Directive #2 is split to a dedicated S3-audio batch
(D-S3-1=B).

**Decisions resolved.**
- D-S3-1=B — audio SFX is its own batch (S3-audio); S3a is visual-only.
- D-S3-2=A — neutral "…" legibility via a TMP outline material on the
  FloatingText prefab (prefab/material edit, applied in Unity).
- D-S3-3=A — SensoryEventBus.Publish iterates the invocation list and
  try/catches each handler, so one throwing subscriber no longer aborts the
  rest (multi-subscriber became real this batch).
- D-S3-4=A — migration (direct-call deletion + adapter Spawn) before the audit.
- D-S3-5=A — song-end FT uses the int SpawnFloatingText overload (xDir=0 →
  randomized ±1) to preserve the S1 random-diagonal drift; the Vector2 overload
  would have forced straight-up and broken parity. Latent defect: S2 verified
  parity only at the payload level (VerifyOnly never spawned), so the drift
  mismatch surfaced at the Spawn flip.
- D-S3-6=A — stage VFX dispatch to per-venue BackgroundRoot.SetSmoke/SetFire;
  stage crossings stay on the direct ActivateSFX path (no SfxStageCrossedEvent
  consumer — additive event deferred until audio needs that signal).
- D-F-5a — restored the audience ability animator trigger (Kid "Tantrum") in
  AudienceCharacterBase.AbilityRoutine, lost in an earlier refactor. D-F-5b (the
  per-loop reaction animator) is deferred: no reaction animator state exists,
  only the ability trigger.
- D-S3-7=A — per-song StartingSongHype is seeded silently (SeedSongHype): stage
  crossings and the SFX→Vibe bonus fire only on performance-driven hype above
  the seed, never during prep. Fixes venue VFX + initial Vibe appearing before
  performance when a threshold sits below the seed.
- D-S3-8=A — both Dev SongHype sliders (DevSetSongHype, OnDebugSongHypeSlider-
  Changed) route through DevSetSongHypeAbsolute and fire stage crossings,
  restoring parity with the ±10% buttons. Pre-existing Dev regression from the
  AddSongHype/AddSongHypeCore split; not introduced by S3a.

**Code changes.**
- GigManager.cs — deleted the loop-reaction and song-end direct FxManager FT
  calls (publishes retained); removed the dead ImpressionToExclamation/
  ImpressionToColor wrappers; added SeedSongHype + StageForNormalized; the seed
  site calls SeedSongHype; added DevSetSongHypeAbsolute and routed both Dev
  setters through it.
- SensoryFxAdapter.cs — default Mode=Spawn; song-end Spawn branch uses the int
  overload.
- SensoryEventBus.cs — per-handler isolation in Publish.
- AudienceCharacterBase.cs — PlayAbilityAnimation + PlayAbilityAnimationRoutine;
  called at the top of AbilityRoutine.
- BackgroundRoot.cs — smokeRoot/fireRoot fields + null-guarded SetSmoke/SetFire.
- BackgroundContainer.cs — ActivateSFX dispatches smoke/fire to the new setters;
  DeactivateAllSFX clears lights+smoke+fire.

**Smoke tests.** ST-S3-1..13 + ST-S3-4 PASS.

**Known follow-ups (not S3a-blocking).** Smoke/fire VFX need an appear/loop
animation (static pop-in contrasts with the rotating lights). The shared
LoopScoreCalculator.cs is stale vs the two-arg config-driven overloads
GigManager calls; refresh it and add the LoopScoringConfig/HypeThresholds struct
file.

Closes. S3a. Opens. S3-audio.

2026-06-14 — S2 SENSORY EVENT BUS FOUNDATION

Closed. S2 introduces a typed sensory event bus between gameplay and its
visual/audio consumers, so future feedback (SFX, animators, tutorial) attaches
as subscribers instead of being hard-wired at each gameplay site. Outcome
matches batch-open intent with two scope adjustments (see Decisions).

**Decisions resolved.**
- D-S2-1=A — bus is a MonoBehaviour singleton with static accessor, mirroring
  FxManager.
- D-S2-2=A — S2 ships two event types only: AudienceReactionEvent +
  SongEndVibeEvent (the S1 sites). Full inventory lands incrementally in S3.
- D-S2-3=A — coexistence: bus publishes fire ALONGSIDE the existing direct
  FxManager calls; direct-call deletion is an S3 task. S1 visuals stay
  bit-identical.
- D-S2-4=A — ISensoryEvent is an empty marker interface; events are readonly
  structs; Publish<TEvent> is generic (zero-alloc, compiler-enforced).
- D-S2-5=defer — TutorialController consumer deferred to S4 (typed Subscribe<T>
  already satisfies the future-consumer requirement; a stub would be dead code).
- D-S2-6=A — a thin SensoryFxAdapter subscribes and converts events to FT;
  FxManager stays a dumb spawner.
- D-S2-7=A — dedup during coexistence: adapter runs in VerifyOnly mode
  (computes + logs the exact FT payload, does not spawn); GigManager direct
  calls remain the sole visual source through S2. Impression→text/colour and
  the song-end FT builder were single-sourced into SensoryFtPresentation so the
  two emission paths cannot drift before S3 deletes the direct calls.
- D-S2-INIT=C — init-order hardening (added mid-batch after intermittent
  missing Verify logs): bus accessor is lazy/auto-creating with
  [DefaultExecutionOrder(-100)] and emits init-confirmation logs. A refinement
  of D-S2-1=A, not a reversal. Root cause was the adapter subscribing once in
  OnEnable while Instance was occasionally null at that instant, with no retry
  and no init logging — silent, intermittent, while bus-independent direct FT
  kept working.

**Code changes.**
- New: `Assets/Scripts/Sensory/` — `ISensoryEvent.cs`, `SensoryEventBus.cs`,
  `SensoryFtPresentation.cs`, `SensoryFxAdapter.cs`; `Sensory/Events/` —
  `AudienceReactionEvent.cs`, `SongEndVibeEvent.cs`.
- `GigManager.cs` — 4-edit patch: `using ALWTTT.Sensory;` parallel publish at
  the loop-reaction site; impression mapping delegated to SensoryFtPresentation;
  song-end site builds the event, derives FT from it via the shared builder, and
  publishes. Direct `FxManager.Instance` spawn calls intact at both sites
  (coexistence; grep-verified present at the loop-reaction and the two song-end
  branches).
- Scene: `SensoryEventBus` on the managers object (with FxManager);
  `SensoryFxAdapter` (VerifyOnly) on the Listeners object in the Gig Scene.

**Smoke tests.** ST-S2-1..7 PASS; ST-S2-9 PASS (init determinism across
repeated play sessions / reloads — the test that retired the intermittency).
ST-S2-8 N/A — its premise ("bus absent in play mode") is no longer reachable
under D-S2-INIT=C lazy auto-init; the publish-site null-guard remains as
defensive dead code. Throwaway `TempSecondSubscriber` used for ST-S2-7
(two-subscriber delivery) then deleted.

**Doc changes.** `planning/active/Design_Sensory_Contract_v0_1.md` §3 (as-built;
coexistence and consumer-table corrections) + §4 (two rows flagged bus+direct;
full audit descoped to S3). `CURRENT_STATE.md` §1/§3/§4/§5. This changelog: S2
entry, S1 entry reordered above CE-L1 (newest-first), S1 neutral-RGB corrected
to (0.55,0.55,0.55) darker-than-MEH (was mis-recorded as 0.75/0.75/0.78).

**Intentionally unchanged.** `SSoT_Runtime_Flow.md` (bus is standalone, does
not register through runtime managers), `SSoT_Scoring_and_Meters.md`,
`SSoT_Audience_and_Reactions.md` (semantics unchanged; §5.1 migration note
optional), `ssot_manifest.yaml`, `coverage-matrix.md`, `SSoT_INDEX.md`. No
authority change.

**Plan-vs-actual.** The §3 S2 next-active row predated D-S2-5/D-S2-3 and
described TutorialController + direct-call migration as S2 work; both moved (S4
and S3 respectively). Corrected at this closure.

**Closes.** S2 (sensory event bus foundation). **Opens.** S3 (sensory polish).

2026-06-12 — S1 (B3-SLATE-F) AUDIENCE REACTIONS RATIFIED + NEUTRAL FT

Closed. S1 is the first implementation batch post-reframe. Outcome differed
from the batch-open expectation: `ResolveLoopEffect`, the taste schema, the
GigManager routing, and the macro-Vibe modifier had already shipped in a
prior B3-slate code batch (tagged [B3 D2=A]/[B3 D3=A] in code) but were
never ratified into SSoTs. S1 therefore = ratification + one small additive
code change + doc backfill.

**Decisions resolved.**
- D-F-1=A — 4-axis discrete-count impression algorithm (TempoScale / role
  count / TimeSignature / Tonality), clamped [−2, +2]. RootNote intentionally
  excluded.
- D-F-2=A — Indifference gates song-end conversion only (ApplyIncomingVibe);
  loop impressions stay live and visible.
- D-F-3=β — macro modifier as impressionFactor = 1 + avg×0.25 ∈ [0.5, 1.5],
  multiplied on SongHype-derived baseVibe at ComputeSongVibeDeltas.
- D-F-4=A — word-exclamation FT (WOW!/YEAH/…/MEH/BORING); neutral "…" added.
- D-F-5=defer — audience reaction animator trigger deferred to S3 (lands
  alongside the pre-existing broken AbilityRoutine trigger consumption).

**Code changes.**
- `GigManager.cs` — `ImpressionToExclamation`: impression=0 now returns "…"
  (was null → no FT); defensive fallthrough also "…".
  `ImpressionToColor`: neutral = muted grey (0.55, 0.55, 0.55), DARKER than
  MEH light grey (0.80, 0.80, 0.80). [Corrected 2026-06-14: S1 originally
  mis-recorded this as light cool grey (0.75, 0.75, 0.78); code truth is
  0.55 neutral, darker than MEH.] Closes the Sensory Contract gap: every
  loop impression now emits FT, including neutral.

**Smoke tests.** ST-S1-1..8 ALL PASS. Highlights: 3-audience
differentiation on the same loop (Cool Dude WOW! vs 2× Kid YEAH on Waltz
Protocol + 1 Sibi track); Indifference per D-F-2=A (loop FT live, song-end
INDIFFERENT floater, no bar fill); Convinced regression clean; neutral "…"
verified via Half Time + 1 Sibi card (Kid raw=0). Known rough edge: neutral
"…" legibility against busy backgrounds — FT outline/shadow deferred to S3
sensory polish. Animator + SFX tests deferred to S3 per D-F-5 / D2
exception.

**Doc changes.**
- `systems/SSoT_Audience_and_Reactions.md` — §6 replaced (CHR/TCH/EMT/SFX
  speculative axes retired; actual 4-axis taste schema documented); §5.1
  rewritten (loop-scale resolution + FT surface + S2 bus migration note);
  new §5.3 (Indifference interaction).
- `systems/SSoT_Scoring_and_Meters.md` — new §6.1 (impressionFactor
  conversion contract; 0.25 coefficient marked tuning-not-contract).
- `planning/active/Design_Sensory_Contract_v0_1.md` — §4 audit rows updated
  (audience reaction row added: FT=yes incl. neutral; vibe-change row
  "post-S1" qualifier removed).
- `CURRENT_STATE.md` — §1 closure block, §3 S1 marked closed / S2 next.

**Intentionally unchanged.** `ssot_manifest.yaml`, `coverage-matrix.md`,
`SSoT_INDEX.md` (no authority change — both edited SSoTs already own these
scopes). `AudienceCharacterData` assets (Kid / Cool Dude taste values are
content, not contract).

**Process note.** The S1 rehydration prompt described `ResolveLoopEffect`
as returning 0 when it had already shipped — the prior code batch closed
without its SSoT backfill and the stale claim propagated into planning.
Standing rule reinforced: no batch closes with code merged and SSoT pending.

**Closes.** S1 (B3-slate-F). **Opens.** S2 (sensory event bus foundation).

## 2026-06-11 — CE-L1: LLM-assisted card authoring arrived in the Card Editor

### Added
- `Assets/Scripts/Cards/LLMAuthoring/` editor-only asmdef pair: shared DTOs +
  parser, vocabulary snapshot builder, palette descriptor scanner,
  palette-intent resolver, prompt builder, generator, response handler, field
  plan; 77 tests + consumer `FakeLLMClient`
- `Assets/Scripts/Cards/Editor/LLM/CardLLMVocabularyBuilder.cs` (Stage 1)
- `Assets/Scripts/Cards/Editor/CardEditorWindow.LLM.cs` — "Generate with LLM"
  panel + Save-step hook (`ApplyLlmPlanOnSave`)
- `reference/Report_CardLLM_Pipeline.md` — explanatory reference for the
  pipeline (not an SSoT)

### Changed
- `CardEditorWindow.JsonImport.cs` (B3): parsing delegated to shared
  `CardImportDtoParser`; `composition.modifierEffectNames` name-resolution
  added to `TryStageCardFromDto`; Save/discard hooks for the LLM field plan
- `systems/SSoT_Editor_Authoring_Tools.md`: new §4.10 (LLM panel + boundary
  rules), §4.3 / §4.5 / §13 / §15 updates
- `systems/SSoT_Card_Authoring_Contracts.md`: new §5.12 (composition-block
  `modifierEffectNames` + `palette` intent) with explicit route scoping — the
  batch JSON box ignores `composition.palette`; §5.3 pointer
- `coverage-matrix.md`: row "Editor authoring tools" note extended

### Key documentary decisions
- Pipeline mechanism documented as **reference**, not SSoT; tool-level truth
  stays in `SSoT_Editor_Authoring_Tools.md`; cross-project pattern authority
  remains MidiGenPlay-side (`authoring/SSoT_Authoring_LLM_Generation.md` §7),
  linked not duplicated
- Asset-reference ban, out-of-alphabet hard-fail, and seeded palette intent
  recorded as normative boundary rules of the LLM route
- Palette intent is LLM-route-only; the plain JSON batch box parses but does
  not resolve it (code-verified 2026-06-12)
